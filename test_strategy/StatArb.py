...
from skywalker.dataseries import aligned
...
from skywalker.tools import yahoofinance
...
class StatArb(strategy.SkysenseBacktestingStrategy):
...
def main(plot):
    ...
    instruments = ['601788.SH', '600005.SH', '000001.SH']
    windowSize = 50
    fromdate = "2015-08-10"
    todate = "2016-08-24"
    # db = pgfeed.Database()
    # db.importBars(instruments, fromdate, todate)
    feed = pgfeed.Feed(frequency=bar.Frequency.DAY)
    # feed.importBars(instruments, fromdate, todate)
    feed.loadBars(instruments, fromDateTime=fromdate, toDateTime=todate)
    ...
    print strat.skysenseAnalyzer.getBeta()
