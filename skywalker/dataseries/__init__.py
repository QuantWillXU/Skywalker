...
from skywalker.utils import collections

DEFAULT_MAX_LEN = 1024 * 10


def series2DataSeries(datalist, datetimelist=None):
    DS = SequenceDataSeries(maxLen=len(datalist))
    if datetimelist is None:
        for i in range(0, len(datetimelist)):
            DS.append(datalist[i])
    else:
        assert len(datalist) == len(datetimelist)
        for i in range(0, len(datetimelist)):
            DS.appendWithDateTime(datetimelist[i], datalist[i])
    return DS
...
class SequenceDataSeries(DataSeries):
    ...
    def getPandasSeries(self):
        values = self.__values[0:]
        datetime = self.getDateTimes()
        series = pd.Series(values, index=datetime)
        return series
