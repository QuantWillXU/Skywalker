...
class BarDataSeries(dataseries.SequenceDataSeries):
    ...
    def getPandasDataFrame(self):
        bardict = {}
        index = ['Open', 'Close', 'High', 'Low', 'Volume', 'AdjClose']
        bardict['Open'] = self.getOpenDataSeries().getPandasSeries()
        bardict['Close'] = self.getCloseDataSeries().getPandasSeries()
        bardict['High'] = self.getHighDataSeries().getPandasSeries()
        bardict['Low'] = self.getLowDataSeries().getPandasSeries()
        bardict['Volume'] = self.getVolumeDataSeries().getPandasSeries()
        bardict['AdjClose'] = self.getAdjCloseDataSeries().getPandasSeries()
        for key in self.__extraDS.keys():
            bardict[key] = self.getExtraDataSeries(key)
            index.append(key)
        df = pd.DataFrame(bardict, columns=index)
        return df
