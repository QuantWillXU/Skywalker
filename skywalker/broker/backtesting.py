...
class Broker(broker.Broker):
    ...
    def __init__(self, cash, barFeed, commission=None):
        ...
        self.__equityEvent = observer.Event()
        self.__equityEvent.subscribe(self.__fixPostionsAndCash)
    ...
    def fixPositions(self, instrument, fixedshares):
        self.__shares[instrument] = fixedshares
    ...
    def __postProcessOrder(self, order, bar_):
        ...
        if not order.getGoodTillCanceled():
            expired = False
            if self.__barFeed.getFrequency() >= skywalker.bar.Frequency.DAY:
                expired = bar_.getDateTime().date() >= order.getAcceptedDateTime().date()
    ...
    def onBars(self, dateTime, bars):
        ...
        ordersToProcess = {k: v for k, v in self.__activeOrders.items()}.values()
    ...
    def dispatch(self):
        ...
        # currentDatetime = self.__barFeed.getCurrentDateTime()
        # self.__equityEvent.emit(currentDatetime)
        pass

    def __fixPostionsAndCash(self, datetime):
        pos = self.getPositions()
        for instrument in pos.keys():
            if self.__barFeed.haveEquityEvent(instrument, datetime):
                equityEvent = self.__barFeed.getEquityEvent(instrument)
                ...
                self.fixPositions(instrument, pos[instrument] * float(1 + div_capitalization + div_stock))
                self.setCash(self.getCash() + pos[instrument] * div_cashaftertax)
