...
class SkysenseSlippage(SlippageModel):
    """"""

    def __init__(self, tickSize=0.01, slippageRate=0):
        super(SkysenseSlippage, self).__init__()
        self.__tickSize = tickSize
        self.__slippageRate = slippageRate

    def setTickSize(self, tickSize):
        self.__tickSize = tickSize

    def setSlippageRate(self, slippageRate):
        self.__slippageRate = slippageRate

    def getTcikSzie(self):
        return self.__tickSize

    def getSlippageRate(self):
        return self.__slippageRate

    def getSlippagePerShare(self):
        ret = self.getSlippageRate() * self.getTcikSzie()
        return ret

    def calculatePrice(self, order, price, quantity, bar, volumeUsed):
        if order.isBuy():
            ret = price + self.getSlippageRate() * self.getTcikSzie()
        else:
            ret = price - self.getSlippageRate() * self.getTcikSzie()
        return ret
